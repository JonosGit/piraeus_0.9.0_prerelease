...
namespace Piraeus.Grains.Notifications
{
    public class DataLakeSink : EventSink
    {
        ...
        public override async Task SendAsync(EventMessage message)
        {
            ...
            {                
                ...
                {                    
                    ...
                    record = new AuditRecord(msg.MessageId, uri.Query.Length > 0 ? uri.ToString().Replace(uri.Query, "") : uri.ToString(), "AzureDataLake", "AzureDataLake", payload.Length, MessageDirectionType.Out, true, DateTime.UtcNow);
                }
            }
            catch (Exception ex)
            {
                Trace.TraceWarning("Initial data lake write error {0}", ex.Message);
                record = new AuditRecord(mid, uri.Query.Length > 0 ? uri.ToString().Replace(uri.Query, "") : uri.ToString(), "AzureDataLake", "AzureDataLake", len, MessageDirectionType.Out, false, DateTime.UtcNow, ex.Message);
            }
            finally
            {
        ...
        {
           //Microsoft.Azure.DataLake.Store.RetryPolicies.ExponentialRetryPolicy policy = new Microsoft.Azure.DataLake.Store.RetryPolicies.ExponentialRetryPolicy(8, 1000);
            ...
            //var creds = GetCreds_SPI_SecretKeyAsync(domain, ADL_TOKEN_AUDIENCE, appId, secret);

            //clients = new DataLakeStoreFileSystemManagementClient[clientCount];
            lastLogin = DateTime.UtcNow;
            clients = new AdlsClient[clientCount];
            
            for (int i = 0; i < clientCount; i++)
            {
                //clients[i] = new DataLakeStoreFileSystemManagementClient(creds);
